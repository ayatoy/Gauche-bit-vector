...
static inline int rank1(u_long b, int i)
{
  ...
  b = ((b & 0xaaaaaaaaUL) >>  1) + (b & 0x55555555UL);
  b = ((b & 0xccccccccUL) >>  2) + (b & 0x33333333UL);
  b = ((b & 0xf0f0f0f0UL) >>  4) + (b & 0x0f0f0f0fUL);
  b = ((b & 0xff00ff00UL) >>  8) + (b & 0x00ff00ffUL);
  b = ((b & 0xffff0000UL) >> 16) + (b & 0x0000ffffUL);
  return b;
#else
  b = i ? (b << (SCM_WORD_BITS - i)) : 0;
  b = ((b & 0xaaaaaaaaaaaaaaaaUL) >>  1) + (b & 0x5555555555555555UL);
  b = ((b & 0xccccccccccccccccUL) >>  2) + (b & 0x3333333333333333UL);
  b = ((b & 0xf0f0f0f0f0f0f0f0UL) >>  4) + (b & 0x0f0f0f0f0f0f0f0fUL);
  b = ((b & 0xff00ff00ff00ff00UL) >>  8) + (b & 0x00ff00ff00ff00ffUL);
  b = ((b & 0xffff0000ffff0000UL) >> 16) + (b & 0x0000ffff0000ffffUL);
  b = ((b & 0xffffffff00000000UL) >> 32) + (b & 0x00000000ffffffffUL);
  return b;
#endif
}
...
static inline int select1(u_long b, int i)
{
  ...
  u_long b2 = ((b1 & 0xccccccccUL) >> 2) + (b1 & 0x33333333UL);
  ...
  v2 = (b2 >> pos) & 0x000fUL; if (i > v2) { i -= v2; pos +=  4; }
  v1 = (b1 >> pos) & 0x0003UL; if (i > v1) { i -= v1; pos +=  2; }
  v0 = (b  >> pos) & 0x0001UL; if (i > v0) { i -= v0; pos +=  1; }
  return i == 1 && (b & (0x1UL << pos)) ? pos : -1;
#else
  u_long b1 = ((b  & 0xaaaaaaaaaaaaaaaaUL) >>  1) + (b  & 0x5555555555555555UL);
  u_long b2 = ((b1 & 0xccccccccccccccccUL) >>  2) + (b1 & 0x3333333333333333UL);
  u_long b3 = ((b2 & 0xf0f0f0f0f0f0f0f0UL) >>  4) + (b2 & 0x0f0f0f0f0f0f0f0fUL);
  ...
  v3 = (b3 >> pos) & 0x000000ffUL; if (i > v3) { i -= v3; pos +=  8; }
  v2 = (b2 >> pos) & 0x0000000fUL; if (i > v2) { i -= v2; pos +=  4; }
  v1 = (b1 >> pos) & 0x00000003UL; if (i > v1) { i -= v1; pos +=  2; }
  v0 = (b  >> pos) & 0x00000001UL; if (i > v0) { i -= v0; pos +=  1; }
  return i == 1 && (b & (0x1UL << pos)) ? pos : -1;
#endif
}
